...
#        self.epsilon = math.cos(self.alpha*self.t)
        self.epsilon = 1/float(math.exp(0.01*self.t))
    ...
    def createQ(self, state):
        ...
        if self.learning == True:
            if state not in self.Q.keys():
                tempQ = {action:0.0 for action in self.valid_actions}
                self.Q[state]=tempQ
        return
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            if self.epsilon < random.uniform(0,1):
                act = []
                for action_list in self.valid_actions:
                    if self.Q[state][action_list] == self.get_maxQ(state):
                        act.append(action_list)
